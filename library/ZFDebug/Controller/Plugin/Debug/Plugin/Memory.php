...
class ZFDebug_Controller_Plugin_Debug_Plugin_Memory extends Zend_Controller_Plugin_Abstract implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getTab()
    {
        if (function_exists('memory_get_peak_usage')) {
            return round(memory_get_peak_usage()/1024) . 'Ko sur '.ini_get("memory_limit");
        }
        return 'MemUsage n.a.';
    }
    ...
    {
        $panel = '<h4>Empreinte m&eacute;moire</h4>';
        $panel .= 'Dispatch: ' . round(($this->_memory['dispatchLoopShutdown']-$this->_memory['dispatchLoopStartup'])/1024,2) .'Ko'.$this->getLinebreak();
        if (isset($this->_memory['user']) && count($this->_memory['user'])) {
            foreach ($this->_memory['user'] as $key => $value) {
                $panel .= $key.': '.round($value/1024).'Ko'.$this->getLinebreak();
            }
        }
        return $panel;
    }
