...
class ZFDebug_Controller_Plugin_Debug_Plugin_Exception implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getTab ()
    {
        ...
        $exception = '';
        if ($errorCount)
            $error = ($errorCount == 1 ? '1 Erreur' : $errorCount . ' Erreurs');
    ...
    public function getPanel()
    {
        ...
        foreach ($response->getException() as $e) {
            $html .= '<h4>Exception ' . get_class($e) . ': ' . $e->getMessage() . '</h4><p>lev&eacute;e dans ' . $e->getFile() . ' &agrave; la ligne ' . $e->getLine() . '</p>';
            $html .= '<h4>Pile d\'ex&eacute;cution</h4><ol>';
            foreach ($e->getTrace() as $t) {
                ...
                if (! isset($t['line']))
                    $t['line'] = 'n/a';
                $html .= '<li>' . $func . '<br>dans ' . str_replace($_SERVER['DOCUMENT_ROOT'], '', $t['file']) . ' &agrave; la ligne ' . $t['line'] . '</li>';
            }
            $html .= '</ol>';
        }
        if ($errorCount) {
            $html .= '<h4>Erreurs</h4><ol>';
            foreach (self::$errors as $error) {
                $html .= '<li>' . sprintf("%s: %s in %s &agrave; la ligne %d", $error['type'], $error['message'], str_replace($_SERVER['DOCUMENT_ROOT'], '', $error['file']), $error['line']) . '</li>';
            }
            $html .= '</ol>';
        }
        return $html;
