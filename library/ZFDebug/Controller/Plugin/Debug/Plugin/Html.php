...
class ZFDebug_Controller_Plugin_Debug_Plugin_Html extends ZFDebug_Controller_Plugin_Debug_Plugin implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getPanel()
    {
        ...
        $panel = '<h4>Informations sur le code (X)HTML</h4>';
        $panel .= $this->_isXhtml().'
        ...
        $panel .= '<span id="ZFDebug_Html_Tagcount"></span> balises'.$this->getLinebreak()
                . 'Poids du code : '.round(strlen($body)/1024, 2).'Ko'.$this->getLinebreak()
                . '<span id="ZFDebug_Html_Stylecount"></span> feuille(s) de style'.$this->getLinebreak()
                . '<span id="ZFDebug_Html_Scriptcount"></span> fichier(s) Javascript'.$this->getLinebreak()
                . '<span id="ZFDebug_Html_Imgcount"></span> images'.$this->getLinebreak()
                . '<form method="post" action="http://validator.w3.org/check"><p><input type="hidden" name="fragment" value="'.htmlentities($body).'"'.$this->getClosingBracket().'<input type="submit" value="Valider avec l\'outil W3C"'.$this->getClosingBracket().'</p></form>';
        return $panel;
    }
}